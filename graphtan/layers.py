...
class TemporalAttentionLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        temporal_inputs = inputs + self.position_embeddings[position_inputs] # [N, T, F] #加入位置embedding
        ...
        outputs = torch.cat(torch.split(outputs, split_size_or_sections=int(outputs.shape[0]/self.n_heads), dim=0), dim=2) # [N, T, F] 对16个头进行拼接
